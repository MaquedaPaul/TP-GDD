...
IF EXISTS (SELECT name FROM sys.tables WHERE name = 'BI_HECHOS_PAGO_ALQUILERES')
DROP TABLE AMCGDD.BI_HECHOS_PAGO_ALQUILERES

IF EXISTS (SELECT name FROM sys.tables WHERE name = 'BI_HECHOS_ALQUILERES')
DROP TABLE AMCGDD.BI_HECHOS_ALQUILERES
...
IF EXISTS(SELECT [name] FROM sys.procedures WHERE [name] = 'MIGRACION_BI_HECHOS_PAGO_VENTA')
DROP PROCEDURE AMCGDD.MIGRACION_BI_HECHOS_PAGO_VENTA

GO

IF EXISTS(SELECT [name] FROM sys.procedures WHERE [name] = 'MIGRACION_PAGO_ALQUILERES')
DROP PROCEDURE AMCGDD.MIGRACION_PAGO_ALQUILERES
GO
IF EXISTS(SELECT [name] FROM sys.procedures WHERE [name] = 'MIGRACION_BI_HECHOS_ALQUILERES')
DROP PROCEDURE AMCGDD.MIGRACION_BI_HECHOS_ALQUILERES
...
IF EXISTS (SELECT [name] FROM sys.views WHERE [name] = 'top5BarriosPorRangoEtario')
    DROP VIEW AMCGDD.top5BarriosPorRangoEtario
GO
...
CREATE TABLE AMCGDD.BI_HECHOS_ALQUILERES (
	
	RANGO_ETARIO NVARCHAR(20),
	TIEMPO_ID NUMERIC(19,0),
	BARRIO_ID NUMERIC(19,0),
	CANTIDAD_ALQUILERES NUMERIC(19,0)
);


CREATE TABLE AMCGDD.BI_HECHOS_PAGO_ALQUILERES (
...
ALTER TABLE AMCGDD.BI_HECHOS_ALQUILERES
ADD CONSTRAINT FK_HECHOS_ALQUILERES_RANGO_ETARIO
FOREIGN KEY (RANGO_ETARIO) REFERENCES AMCGDD.BI_DIMENSION_RANGO_ETARIO (RANGO_ETARIO_ID)

ALTER TABLE AMCGDD.BI_HECHOS_ALQUILERES
ADD CONSTRAINT FK_HECHOS_ALQUILERES_TIEMPO
FOREIGN KEY (TIEMPO_ID) REFERENCES AMCGDD.BI_DIMENSION_TIEMPO (TIEMPO_ID)

ALTER TABLE AMCGDD.BI_HECHOS_ALQUILERES
ADD CONSTRAINT FK_BI_PAGO_VENTA_BARRIO
FOREIGN KEY (BARRIO_ID) REFERENCES AMCGDD.BI_BARRIO (BARRIO_ID)

-- CONSTRAINT HECHOS PAGO ALQUILERES

ALTER TABLE AMCGDD.BI_HECHOS_PAGO_ALQUILERES
...
CREATE PROCEDURE AMCGDD.MIGRACION_PAGO_ALQUILERES
AS
BEGIN
	
	INSERT INTO BI_HECHOS_PAGO_ALQUILERES
	SELECT
		dt.TIEMPO_ID AS TIEMPO,
	...
	GROUP BY dt.TIEMPO_ID, ea.estado_nombre 
...
CREATE VIEW AMCGDD.top5BarriosPorRangoEtario AS
SELECT subquery.RANGO_ETARIO, subquery.ANIO, subquery.CUATRIMESTRE, subquery.BARRIO, subquery.CANTIDAD_ALQUILERES
FROM (
    SELECT RANGO_ETARIO, ANIO, CUATRIMESTRE, BARRIO, CANTIDAD_ALQUILERES,
        ROW_NUMBER() OVER(PARTITION BY RANGO_ETARIO ORDER BY CANTIDAD_ALQUILERES DESC) AS puesto
    FROM (
        SELECT a.rango_etario AS RANGO_ETARIO, dt.anio AS ANIO, dt.CUATRIMESTRE AS CUATRIMESTRE, a.barrio_id AS BARRIO, a.CANTIDAD_ALQUILERES AS CANTIDAD_ALQUILERES
        FROM AMCGDD.BI_HECHOS_ALQUILERES a
        JOIN AMCGDD.BI_DIMENSION_TIEMPO dt
        ON a.tiempo_id = dt.TIEMPO_ID
    ) AS datos_ordenados
) AS subquery
WHERE subquery.puesto <= 5
go
...
FROM AMCGDD.BI_HECHOS_PAGO_ALQUILERES AS pa
...
AS
SELECT dt.ANIO ANIO, dt.MES AS MES, 100*SUM(pa.INCREMENTO_MES_ANTERIOR)/SUM(pa.TOTAL_PAGO_MES) AS PORCENTAJE_INCREMENTO
FROM AMCGDD.BI_HECHOS_PAGO_ALQUILERES AS pa
JOIN AMCGDD.BI_DIMENSION_TIEMPO AS dt
	ON pa.tiempo_id = dt.TIEMPO_ID
GROUP BY dt.ANIO , dt.MES
...
EXEC AMCGDD.MIGRACION_PAGO_ALQUILERES
EXEC AMCGDD.MIGRACION_BI_HECHOS_PAGO_VENTA