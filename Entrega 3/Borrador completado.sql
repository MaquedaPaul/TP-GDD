...
ALTER TABLE AMCGDD.BI_HECHOS_PAGO_ALQUILERES
ADD CONSTRAINT FK_BI_PAGO_ALQUILERES_MONEDA
FOREIGN KEY (MONEDA) REFERENCES AMCGDD.BI_DIMENSION_TIPO_MONEDA (MONEDA_NOMBRE)
...
SELECT dt.ANIO, dt.CUATRIMESTRE, alq.RANGO_ETARIO, ba.BARRIO_ID
FROM AMCGDD.BI_HECHOS_ALQUILERES alq
JOIN AMCGDD.BI_DIMENSION_TIEMPO dt ON alq.TIEMPO_ID = dt.TIEMPO_ID
JOIN AMCGDD.BI_BARRIO ba ON alq.BARRIO_ID = ba.BARRIO_ID
WHERE ba.BARRIO_ID IN 
	(SELECT TOP 5 ba2.BARRIO_ID FROM AMCGDD.BI_HECHOS_ALQUILERES alq2
	 JOIN AMCGDD.BI_DIMENSION_TIEMPO dt2 ON alq.TIEMPO_ID = dt2.TIEMPO_ID
	 JOIN AMCGDD.BI_BARRIO ba2 ON ba2.BARRIO_ID = alq2.BARRIO_ID
	 GROUP BY ANIO, CUATRIMESTRE, RANGO_ETARIO, ba2.BARRIO_ID
	 ORDER BY SUM(CANTIDAD_ALQUILERES) DESC) 
GROUP BY ANIO, CUATRIMESTRE, RANGO_ETARIO, ba.BARRIO_ID
go
...
EXEC AMCGDD.MIGRACION_BI_HECHOS_ALQUILERES
