...
ALTER TABLE AMCGDD.PROPIETARIO-------------PROP
ADD CONSTRAINT FK_PROPIETARIO
FOREIGN KEY (PROPIETARIO_ID) REFERENCES AMCGDD.PROPIETARIO
...
BEGIN
	...
	VENTA_PRECIO)
    SELECT DISTINCT
	AMCGDD.ID_COMPRADOR(COMPRADOR_DNI),
	ANUNCIO_CODIGO,
	VENTA_CODIGO, 
	VENTA_COMISION, 
	VENTA_FECHA, 
...
BEGIN
	...
	SELECT DISTINCT usuario_id, sucursal_id FROM 
	...
	JOIN AMCGDD.SUCURSALES s
	ON s.sucursal_id = SUCURSAL_CODIGO
...
BEGIN
	...
	ON m.MONEDA_NOMBRE = CONCAT(UPPER(LEFT(SUBSTRING(ANUNCIO_MONEDA, 8, LEN(ANUNCIO_MONEDA) - 7),1)),LOWER(SUBSTRING(ANUNCIO_MONEDA, 9, LEN(ANUNCIO_MONEDA) - 1)))
...
AS
  BEGIN
	...
	ESTADO_ID,TIPO_ID, ORIENTACION_ID, DISPOSICION_ID, BARRIO_ID, PROPIETARIO_ID)
		...
		(SELECT TOP 1 BARRIO.BARRIO_ID FROM AMCGDD.BARRIO WHERE BARRIO.BARRIO_NOMBRE = INMUEBLE_BARRIO) AS BARRIO_ID
...
CREATE PROCEDURE AMCGDD.MIGRACION_INMUEBLE_PROPIETARIO
 AS
  BEGIN
	PRINT '**MIGRACION** MIGRACION_INMUEBLE_PROPIETARIO'
	INSERT INTO INMUEBLE(PROPIETARIO_ID)
    SELECT DISTINCT ALQUILER_CODIGO, ANUNCIO_CODIGO, ALQUILER_FECHA_INICIO, ALQUILER_FECHA_FIN, ALQUILER_CANT_PERIODOS,
            ALQUILER_DEPOSITO, ALQUILER_COMISION, ALQUILER_GASTOS_AVERIGUA, inquilino_usuario, 1
        FROM gd_esquema.Maestra 
        JOIN AMCGDD.USUARIOS u
        ...
        ON u.usuario_dni = INQUILINO_DNI
            AND    u.usuario_nombre = RTRIM(LTRIM(UPPER(INQUILINO_NOMBRE)))
            AND    u.usuario_apellido = RTRIM(LTRIM(UPPER(INQUILINO_APELLIDO)))
        JOIN AMCGDD.PROPIETARIOS i
        ON i.propietario_usuario = u.usuario_id
END
GO
